import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class imageCountBW_0tt extends PApplet {// white #FFFFFF against black #000000
//by luis blackaller
...
int randomImage = PApplet.toInt(random(0f,3.99f));
...
public void draw() { 
  ...
  if( pic1.pixels[stepX] == 0xffFFFFFF){ 
      numWhite = numWhite + 1;
      fill(255);
  ...
  if( pic1.pixels[stepX] == 0xff000000){ 
      numBlack = numBlack + 1;
      fill(0);
    ...
    //text("white precentage: "+numWhite, 17*width/24, 50);
      ...
      text("white:"+nfs(PApplet.toFloat(100*numWhite)/PApplet.toFloat(pic1.pixels.length),2,4)+" %", 14*width/24, 48);  
    //}
    //if(numBlack>0){
    //fill(255);
    ...
    text("black:"+nfs(PApplet.toFloat(100*numBlack)/PApplet.toFloat(pic1.pixels.length),2,4)+" %", 14*width/24, 62); 
    }
...
static public void main(String args[]) {   PApplet.main(new String[] { "imageCountBW_0tt" });}}