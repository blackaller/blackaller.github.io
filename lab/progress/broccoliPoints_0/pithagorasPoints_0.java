import processing.core.*; import java.util.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class pithagorasPoints_0 extends PApplet {//import processing.opengl.*;
...
float depth = 6, x0,y0; // dont go beyond 6. 5 is nice
...
public void draw() {
  ...
  if(( angle <= 0.8f) && (crecer ==true))
    {angle = (angle + (0.01f/cont));
     //depth = depth+0.3f;
    
    if( angle == 0.8f) crecer = false;
...
public void broccoli(Point2D A, Point2D B, float depth, float angle){ // recursive method
 ...
 //trivial con el isosceles E = new Point2D( A.x -u1   + 0.5f*( u0 - u1 ) , A.y + u0  + 0.5f*( u1 + u0 ) );
  ...
  if (depth > 1){
     depth = depth-1;
     broccoli(C, A, depth, angle);
     broccoli(B, D, depth, angle);
