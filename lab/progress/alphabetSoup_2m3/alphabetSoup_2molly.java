import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class alphabetSoup_2molly extends PApplet {// information. reading a simple relation. alphabet city.
...
public void draw()
{
  ...
  letra = PApplet.toChar(book[lector]);
  siguiente = PApplet.toChar(book[(lector+1) % book.length]);
  ...
  {  
   ...
   for ( int j=0; j<num; j++ ){
    ...
    fill(0);
    textFont(font, 10); 
    text(PApplet.toChar(book[lector+j]), (j+1)*10, height -10);   
...
public void conexion(Pelota A, Pelota B){
  float mirand1 = random(-1,1);
  float mirand2 = random(-1,1);
  int cradio = 4;

  stroke(0,50);
  line (A.x + A.radio*mirand1/cradio, A.y+A.radio*mirand1/cradio, B.x+B.radio*mirand2/cradio, B.y+B.radio*mirand2/cradio);

}

// hacer una clase conector(Alambre). una vez que dos nodos est\u00e1n relacionados conserva esa relaci\u00f3n. y tiene su propio radio y su propio m\u00e9todo display,
...
public Point2D coordenadas(int i){
...
class Pelota{
  ...
  Pelota(){
   // radio = width/(2*alfabeto.length());
    radio = 1.5f*width/(alfabeto.length());
    masa = radio;
  ...
  public void display(){
    ...
    textFont(font, radio*1.2f); // good for in ellipse
    text(letra, x-radio/3, y+radio/2.5f);   // good for in ellipse
