...
int stepW=0;
int stepB=0;
int imageChoice = int(random(0f,2.999f));
//int imageChoice = 2;
...
void draw() { 
  ...
  if((stepX !=0)&&(stepX % (width/2) == 0))stepY=stepY+1;
    ...
    //text("white precentage: "+numWhite, 17*width/24, 50);
      ...
      text("bright:"+nfs(float(100*numWhite)/float(pic1.pixels.length),2,4)+" %", 14*width/24, 48);  
    //}
    //if(numBlack>0){
    //fill(255);
    ...
    text("dark:"+nfs(float(100*numBlack)/float(pic1.pixels.length),2,4)+" %", 14*width/24, 62); 
    }
