import processing.core.*; import java.util.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class polygonDraw_1b extends PApplet {//import processing.opengl.*;
	...
	{	Point2D ptemp = new Point2D(0,0);
                ...
                if (j+1<=255) 
                  //fill(int( 255 * ptemp.x ),int( 255 * ptemp.x ),int( 255 * ptemp.x ),j+20);
                   fill(0,0,PApplet.toInt( 255 * ptemp.x ),j+20);
                else  
                  fill(0,0,PApplet.toInt( 255 * ptemp.x ));
...
{ float xA = mouseX, yA = mouseY;
  ...
  if( v.size() > 0 && dx * dx + dy * dy < 5)
    ...
    vrand.add(new Point2D ( random(.3f,1), 0) );
  } 
}
