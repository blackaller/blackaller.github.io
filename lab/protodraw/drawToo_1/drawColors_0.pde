...
PFont font;
int cell = 1;
color miColor = (color)random(#000000, #FFFFFF);
boolean start =  true;
...
void setup(){
  ...
  void draw(){
    ...
    if (mousePressed){
      if(start == true){ background(#DDDDCC); start = false;}
      pxLine(pmouseX,mouseX, pmouseY,mouseY, cell);}
    
    //pxLine(200,mouseX, 200,mouseY);
    
    if (keyPressed) {
      miColor = (color)random(#000000, #FFFFFF);
      ...
      if (key == '0') {miColor = #FFFFFF;fill(miColor);}
     ...
     void pxLine( int x0, int x1, int y0, int y1, int cell){
     ...
     void swap (int x, int y){ // doesnt work
       int tempX =x;
       int tempY =y;
