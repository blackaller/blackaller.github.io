import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class alphabetSoup_1a extends PApplet {// information. reading simple relationships.
...
float elasticidad = 0.95f;   //.95                       
float friccion = 0.75f;   //.75
...
public void setup()
{
  ...
  for ( int i=0; i<num; i++ )
  {
    ...
    pelotas[i].x = (i+.75f)*2*(pelotas[i].radio); // i+.75 for w=800, i+1.75 for w = 400.
...
public void draw()
{
  ...
  {  
    ...
    if ((letra == pelotas[i].letra) && (muchoRadio)) {pelotas[i].radio = pelotas[i].radio+.4f;} // este incremento lee 67210 caracteres de 1221069
