import processing.core.*; import java.util.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class pithagorasTree_0 extends PApplet {//import processing.opengl.*;
...
public void draw() {
  ...
  rect(340,125,40,150);
    ...
    if ((A.x >= 125) && (A.x <= 275) && (B.x >= 125) && (B.x <= 275) && (A.y >= 20) && (A.y <= 60) && (B.y >= 20) && (B.y <= 60)) {
      ...
      else{broccoli(A, B, depth,  PI/4.5f);}
      A.display();B.display();
      ...
      if ((A.x >= 125) && (A.x <= 275) && (B.x >= 125) && (B.x <= 275) && (A.y >= 340) && (A.y <= 380) && (B.y >= 340) && (B.y <= 380)) {
        ...
        else{broccoli( B,A, depth,  PI/4.5f);}
        A.display();B.display();
      }
      else{
...
public void broccoli(Point2D A, Point2D B, float depth, float angle){ // recursive method
  Point2D C;
  Point2D D;
  Point2D E;
 ...
 //trivial con el isosceles E = new Point2D( A.x -u1   + 0.5f*( u0 - u1 ) , A.y + u0  + 0.5f*( u1 + u0 ) );
  E = new Point2D( A.x -u1   + 0.4f*( u0 - u1 ) , A.y + u0  + 0.4f*( u1 + u0 ) );
