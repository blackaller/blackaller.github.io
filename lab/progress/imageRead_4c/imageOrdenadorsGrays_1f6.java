import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class imageOrdenadorsGrays_1f6 extends PApplet {// separate bright from dark
...
int imageChoice = PApplet.toInt(random(0f,2.99999f));
//int imageChoice = 3;
  ...
  noLoop();
   ...
   String toGetColors[] =loadStrings("image_"+imageChoice+".txt");
    ...
    for(int i=0;i< picLength;i++){
        ...
        picValues[i]=PApplet.toInt(list[0]);
        order[i]=PApplet.toInt(list[1]);
...
public void draw() { 
  ...
  ellipse( (order[stepX] % (width/2)), (order[stepX]/(width/2)),4,4);     
    ...
    if( picValues[stepX] >= 0xff808080){ 
      numBright = numBright + 1;
    }
      
      
   if( picValues[stepX] < 0xff808080){ //#808080    
      numDark = numDark + 1;
  }
  ...
  if((stepX !=0)&&(stepX % (width/2) == 0))stepY=stepY+1;
    ...
    textFont(font, 10);  
      ...
      text("bright:"+nfs(PApplet.toFloat(100*numBright)/PApplet.toFloat(picLength),2,4)+" %", 14*width/24, 48);  
  
    fill(0);
    textFont(font, 10); 
  
    
    text("dark:"+nfs(PApplet.toFloat(100*numDark)/PApplet.toFloat(picLength),2,4)+" %", 14*width/24, 62); 
    }
...
public int[] ordenadorBrillo2(int[] P){
...
public void ordenadorBrillo(int[] P){
     int n = P.length;
     int menor;
     int primero = indexMinC(P);
...
public void intercambio(int[] P, int a, int b){ //este metodo parece que si funciona
  int n = P.length;
